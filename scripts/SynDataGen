...
for i in range(n):
    ...
    if mes in [11, 12]:  # Black Friday e Natal
        if np.random.rand() < 0.5:  # 50% de chance de criar um pedido extra
            dates[i] = np.random.choice(pd.date_range("2024-11-01", "2024-12-31", freq='h'))
    elif mes in [1, 2]:  # Período de baixa demanda
        if np.random.rand() < 0.3:  # 30% de chance de remover um pedido
            dates[i] = np.random.choice(pd.date_range("2024-01-01", "2024-02-28", freq='h'))

# Centros de distribuição
centros = ["São Paulo (SP)", "Recife (PE)", "Brasília (DF)", "Florianópolis (SC)", "Belém (PA)"]
...
tempo_separacao = np.random.normal(loc=20, scale=5, size=n) + (percentual_ocupacao_CD - 75) * 0.2
...
prob_atraso = np.where(percentual_ocupacao_CD > 85, 0.25, 0.15)  # Maior atraso se ocupação > 85%
...
df.to_csv("dados_logisticos_sinteticos.csv", index=False)
